...
pvd_print = 12 # earlier i was recording (as.pvd files) 3 particles. Now i have increased it to 5 !
if ensemble_comm.rank%pvd_print == 0: # printing results correponding to every pvd_print particle
    ...
    outfile = File('../results/upd_noise_var_90_mesh_32_particle_'+str(ensemble_comm.rank)+'_t27_onwards.pvd', comm = spatial_comm)
...
n_EOF = 51 # desired no. of EOFs to include in stochastic sim
np.random.seed(seed_no)
rand_mat = np.random.normal(size=(n_t_steps+2, n_EOF))

vel_data_sto, temp_data_sto, vort_data_sto = [], [], []
vel_data_sto.append(np.array(u_.at(gridpoints, tolerance=1e-10)))
temp_data_sto.append(np.array(theta_.at(gridpoints, tolerance=1e-10)))
vort_data_sto.append(np.array(vort_.at(gridpoints, tolerance=1e-10)))
...
PETSc.Sys.Print(f'Local time at the start of simulation for particle {ensemble_comm.rank}: {time.strftime("%H:%M:%S", time.localtime())}')
...
while (round(t,4) <= t_end):
    ...
    if iter_n%freq == 0:
        if iter_n == freq:
            ...
            PETSc.Sys.Print(f'approx. running time for one big t_step for particle {ensemble_comm.rank} is {round(execution_time,2)} minutes')
            total_execution_time = ((t_end - t_start)/big_t_step)*execution_time
            PETSc.Sys.Print(f'approx. total running time for particle {ensemble_comm.rank} is {round(total_execution_time,2)} minutes')
        ...
        temp_data_sto.append(np.array(theta.at(gridpoints, tolerance=1e-10)))
        vort_data_sto.append(np.array(vort.at(gridpoints, tolerance=1e-10)))
...
data_file = '../data_from_stochastic_run/vel_temp_vort_data_particle_'+str(seed_no)+'_var_90_grid_32_t27_onwards_decor_t_32min_upd_noise.npz'
np.savez(data_file, gridpoints = gridpoints, vel_data_sto = np.array(vel_data_sto), temp_data_sto = np.array(temp_data_sto), vort_data_sto = np.array(vort_data_sto))

print(f'Local time at the end of simulation for particle {ensemble_comm.rank} is {time.strftime("%H:%M:%S", time.localtime())}')
