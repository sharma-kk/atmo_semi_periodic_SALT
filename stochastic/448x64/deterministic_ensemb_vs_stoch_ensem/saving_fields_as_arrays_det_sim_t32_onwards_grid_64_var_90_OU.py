...
t_comp = 32.0 # time after which we switch off the stochastic terms
...
# we run the stochastic model for 5 time units first. From t = 32 onwards, we switch off the parameterisation terms to get 
# an ensemble of deterministic model solution

data_file = '../ensemble_vel_temp_data_as_arrays/deterministic_ensemble_var_90_mesh_64_particle_'+str(seed_no)+'fields_data_at_t_27.0.npz'
...
while (round(t,4) <= t_end):
    ...
    u_pert.assign(0)
    if round(t,4) <= t_comp: # we use stochastic parameterisation only till t = t_comp
    ...
    if iter_n%freq == 0:
        ...
        data_file = '../ensemble_vel_temp_data_as_arrays/deterministic_ensemble_var_90_mesh_64_particle_'+str(seed_no)+'fields_data_at_t_'+str(round(t,4))+'.npz'
