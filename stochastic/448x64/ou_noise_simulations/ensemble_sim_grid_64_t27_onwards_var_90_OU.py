...
Ro = 0.3 ; Re = 3*10**4 ; C = 0.02 ; Pe = 3*10**4
...
if ensemble_comm.rank%pvd_print == 0: # printing results correponding to every pvd_print particle
    ...
    outfile = File('../results/OU_sim_var_90_mesh_64_particle_'+str(ensemble_comm.rank)+'_t27_onwards.pvd', comm = spatial_comm)
...
xi_data = np.load('../../../deterministic/448x64/xi_calculation_visualization/xi_vec_data/xi_matrix_combined_calc_94_eigvec_c_1_by_64_decor_t_16min_t=27_to_t=45.npz')
...
PETSc.Sys.Print(f'seed_no for rank {ensemble_comm.rank} is {seed_no}, local time: {time.strftime("%H:%M:%S", time.localtime())}')

n_t_steps = int((t_end - t_start)/Dt)
n_EOF = 94 # desired no. of EOFs to include in stochastic sim
##############
# 94 EOFs ---------> 90 % variance
# 39 EOFs ---------> 70 % variance
# 18 EOFs ---------> 50 % variance 
...
acf_data = np.load('../../../deterministic/448x64/acf_data/acf_1_data_94_EOFs_mesh_64.npz')
...
while (round(t,4) <= t_end):
    ...
    if iter_n%freq == 0:
        ...
        data_file = '../data_from_stochastic_run/vel_temp_vort_data_particle_'+str(seed_no)+'_var_90_grid_64_t27_onwards_OU.npz'
